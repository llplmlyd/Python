...
#在spider1的基础上添加了user_agent,这里设置为web spider with python的缩写
	...
	# print(type(request))
	#声明headers=字典值的headers。用Request类构建了一个完整的请求，增加了headers等一些信息,相当于改造url
	try: 
		html=urllib.request.urlopen(req).read()
	 #尝试下载网页并返回其HTML
	# except HTTPError as e: #将HTTP的错误存储到e值中
	# 	print('HTTPError下载出错：', e.code)
	# 	html=None
	# except HTTPError as e:  #将HTTP的5xx错误存储到e值中
	# 	html=None
	# 	if num_retries>0:# 默认最大的下载次数为2次，失败后可以重新再下载两次
	# 		if hasattr(e, 'getcode') and 500<=e.code<=600: 
	# 		#hasattr 用来检查e值是否带有code属性，并且检查code值是否在500-600之间的URLError
	# 		#如果满足条件则尝试再次下载
	# 			print('URLError下载出错,尝试再次下载：', e.code)
	# 			num_retries=num_retries-1
	# 			return download(url,num_retries,user_agent) #循环函数，下载次数减一
	# 		else:
	# 			print('HTTPError下载出错：', e.code)
